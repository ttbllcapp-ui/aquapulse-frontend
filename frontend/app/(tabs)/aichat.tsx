...
import { View, Text, StyleSheet, TouchableOpacity, ScrollView, TextInput, KeyboardAvoidingView, Platform, ActivityIndicator, Alert, Modal, Pressable, Linking } from 'react-native';
import { LinearGradient } from 'expo-linear-gradient';
import { SafeAreaView } from 'react-native-safe-area-context';
import { Sparkles, Send, Trash2, MessageCircleQuestion, Plus, X, Info, ShieldAlert, BookOpen } from 'lucide-react-native';
...
const DISCLAIMER_KEY = '@aquapulse/aichat_disclaimer_seen_v1';

const SOURCES: { name: string; url: string }[] = [
  { name: 'World Health Organization (WHO)', url: 'https://www.who.int/news-room/fact-sheets/detail/drinking-water' },
  { name: 'Mayo Clinic', url: 'https://www.mayoclinic.org/healthy-lifestyle/nutrition-and-healthy-eating/in-depth/water/art-20044256' },
  { name: 'Healthline', url: 'https://www.healthline.com/nutrition/how-much-water-should-you-drink-per-day' },
];
...
export default function AIChatScreen() {
  ...
  const [disclaimerOpen, setDisclaimerOpen] = useState(false); // one-time on first visit
  const [infoOpen, setInfoOpen] = useState(false); // manual open via ⓘ button
  ...
  // One-time disclaimer modal on first AquaCoach visit
  useEffect(() => {
    (async () => {
      try {
        ...
        // If storage fails, still show disclaimer (safer fallback)
    ...
    <View style={[styles.root, { backgroundColor: palette.bgPrimary }]}>
      ...
      <SafeAreaView style={{ flex: 1 }} edges={['top']}>
        <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : undefined} keyboardVerticalOffset={Platform.OS === 'ios' ? 0 : 0}>
              ...
              <View style={{ flex: 1 }}>
                <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6 }}>
                  ...
                  <TouchableOpacity
                    ...
                    activeOpacity={0.7}
                    accessibilityLabel={lang === 'tr' ? 'Bilgi ve kaynaklar' : 'Info and sources'}
                  >
                    <Info color={palette.textSecondary} size={18} strokeWidth={2} />
                  </TouchableOpacity>
                </View>
          ...
          {/* Always-visible citations banner — below header, above messages */}
          <View
            testID="citations-banner"
            style={[
              styles.citationsBanner,
              { backgroundColor: palette.glassBg, borderColor: palette.borderLight },
            ]}
          >
            <View style={styles.citationsRow}>
              <Info color={palette.textSecondary} size={13} />
              {SOURCES.map((s) => (
                <TouchableOpacity
                  key={s.name}
                  testID={`cite-${s.name}`}
                  onPress={() => openSourceUrl(s.url)}
                  hitSlop={{ top: 6, bottom: 6, left: 4, right: 4 }}
                  activeOpacity={0.6}
                >
                  <Text style={[styles.citationLink, { color: palette.primary }]}>
                    {s.name.replace('World Health Organization (WHO)', 'WHO')}
                  </Text>
                </TouchableOpacity>
              ))}
            </View>
            <Text style={[styles.citationDisclaimer, { color: palette.textSecondary }]} numberOfLines={1}>
              {lang === 'tr'
                ? 'Bilgiler eğitim amaçlıdır. Tıbbi tavsiye değildir.'
      ...
      {/* One-time Disclaimer Modal — shown on first AquaCoach visit */}
      <Modal
      ...
      >
        <View style={styles.modalRoot}>
          <View style={[styles.confirmCard, { backgroundColor: palette.bgCard, borderColor: palette.borderLight }]}>
            <View style={[styles.modalIconWrap, { backgroundColor: `${palette.primary}22` }]}>
              <ShieldAlert color={palette.primary} size={28} />
            </View>
            <Text style={[styles.modalTitle, { color: palette.textPrimary }]}>
              {lang === 'tr' ? 'Önemli Bilgi' : 'Important Notice'}
            </Text>
            <Text style={[styles.modalBody, { color: palette.textSecondary, marginTop: 10 }]}>
              {lang === 'tr'
                ? 'Bu uygulama tıbbi tavsiye vermez. AquaCoach yalnızca genel bilgilendirme amaçlıdır. Sağlık sorularınız için lütfen bir doktora başvurun.'
                : 'This app does not provide medical advice. AquaCoach is for general information only. Please consult a doctor for any health concerns.'}
            </Text>
            <View style={[styles.sourceBox, { backgroundColor: palette.glassBg, borderColor: palette.borderLight }]}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 4 }}>
                <BookOpen color={palette.primary} size={14} />
                <Text style={{ fontSize: 12, fontWeight: '700', color: palette.textPrimary }}>
                  {lang === 'tr' ? 'Kaynaklar' : 'Sources'}
                </Text>
              </View>
              <Text style={{ fontSize: 12, color: palette.textSecondary, lineHeight: 17 }}>
      ...
      {/* Info Modal — opened via ⓘ button (re-openable) */}
      <Modal
        visible={infoOpen}
        transparent
        animationType="fade"
      ...
      >
        <Pressable style={styles.modalRoot} onPress={() => setInfoOpen(false)}>
          <Pressable
            onPress={(e) => e.stopPropagation()}
            style={[styles.infoCard, { backgroundColor: palette.bgCard, borderColor: palette.borderLight }]}
          >
            <View style={{ flexDirection: 'row', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
              <View style={{ flexDirection: 'row', alignItems: 'center', gap: 8 }}>
                <View style={[styles.modalIconWrapSmall, { backgroundColor: `${palette.primary}22` }]}>
                  <Info color={palette.primary} size={18} />
                </View>
                <Text style={[styles.modalTitle, { color: palette.textPrimary, marginTop: 0 }]}>
                  {lang === 'tr' ? 'Bilgi & Kaynaklar' : 'Info & Sources'}
                </Text>
              </View>
              <TouchableOpacity
                testID="close-info"
            ...
            <Text style={[styles.infoSectionTitle, { color: palette.textPrimary }]}>
              {lang === 'tr' ? 'Tıbbi Tavsiye Reddi' : 'Medical Disclaimer'}
            </Text>
            <Text style={[styles.infoText, { color: palette.textSecondary }]}>
              {lang === 'tr'
                ? 'AquaCoach tıbbi tavsiye, tanı veya tedavi sağlamaz. Bilgiler yalnızca eğitim amaçlıdır. Sağlık sorularınız için lütfen bir hekime danışın.'
                : 'AquaCoach does not provide medical advice, diagnosis, or treatment. The information is for educational purposes only. Please consult a physician for health-related questions.'}
            </Text>

            <Text style={[styles.infoSectionTitle, { color: palette.textPrimary, marginTop: 14 }]}>
              {lang === 'tr' ? 'Güvenilir Kaynaklar' : 'Trusted Sources'}
            </Text>
            <View style={{ gap: 8, marginTop: 6 }}>
              {SOURCES.map((s) => (
                <TouchableOpacity
                  key={s.name}
                  testID={`source-${s.name}`}
                  activeOpacity={0.75}
                  onPress={() => openSourceUrl(s.url)}
                  style={[styles.sourceItem, { backgroundColor: palette.glassBg, borderColor: palette.borderLight }]}
                >
                  <BookOpen color={palette.primary} size={16} />
                  <Text style={{ flex: 1, color: palette.textPrimary, fontSize: 13, fontWeight: '600' }}>
                    {s.name}
                  </Text>
...
const styles = StyleSheet.create({
  ...
  // Disclaimer + Info modal styles
  sourceBox: { width: '100%', marginTop: 12, padding: 10, borderRadius: 12, borderWidth: 1 },
  btnPrimary: { paddingVertical: 13, borderRadius: 999, alignItems: 'center' },
  infoCard: { width: '100%', maxWidth: 380, padding: 18, borderRadius: 22, borderWidth: 1 },
  modalIconWrapSmall: { width: 32, height: 32, borderRadius: 16, alignItems: 'center', justifyContent: 'center' },
  infoSectionTitle: { fontSize: 13, fontWeight: '800', marginTop: 12 },
  infoText: { fontSize: 13, lineHeight: 19, marginTop: 4 },
  sourceItem: { flexDirection: 'row', alignItems: 'center', gap: 10, paddingHorizontal: 12, paddingVertical: 11, borderRadius: 12, borderWidth: 1 },
  // Always-visible citations banner (below header)
  citationsBanner: {
    marginHorizontal: 16,
    marginTop: 6,
    marginBottom: 8,
